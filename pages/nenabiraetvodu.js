  ...
  const sections = [
    {
      ...
      imageAlt: '',
      imageCaption: '',
      content: `<p><strong>1. Выключите питание и закройте кран.</strong><br/>Прежде чем проверять узлы машины, отключите её от электросети и перекройте подачу воды. Так вы обезопасите себя и ��ехнику.</p>
    ...
    },
    {
      ...
      content: `<p>Водяной кран – это вентиль на трубе, подающей воду к машинке. Если он закрыт или напор воды очень слабый, вода в машину просто не поступает. Признак такой неисправности: машина пытается начать работу (гудит), но вода не набирается или идёт очень ме��ленно. Убедитесь, что вентиль полностью открыт (ручка параллельна трубе).Если напор в кране слабый, попробуйте подождать стабилизации давления или сообщить об этом в ЖКХ. Если кран сломан или не держит герметичность, вызовите сантехника или мастера.</p>`
    },
    {
      id: '3',
    ...
    },
    {
      ...
      content: `<p>Прессостат – датчик, определяющий уровень во��ы в баке. Если прессостат «залип» (считал бак полным) или вышел из строя, машина не начнёт залив воды вообще. Симптомы схожи с клапаном: люк блокируется, но залива нет, появляется ошибка «набор воды» или «прессостат» на дисплее. Иногда вода может сливаться сразу, создаётся ложная сигнализация об избыточном уровне. Пользователь может проверить шланг прессостата на засор (и прочистить его), но обычно требуется замена датчика.</p>`
    },
    {
      id: '5-2',
