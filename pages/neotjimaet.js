...
const Page = () => {
  ...
  const anchors = {
    ...
    items: [
      ...
      { icon: '/external/vector8669-r6fr.svg', href: '#5', text: '4. Растянутый или порвавшийся приводной ремен��' },
      { icon: '/external/vector8669-r6fr.svg', href: '#5-1', text: '5. Неисправность двигателя' },
      { icon: '/external/vector8669-r6fr.svg', href: '#5-2', text: '6. Поло��ка таходатчика (датчика оборотов)' },
    ],
  }
  ...
  const sections = [
    {
      ...
      content: `<p><strong>1. Для начала проверьте сливной фильтр.</strong><br/>Он удерживает мелкие предметы — монеты, пуговицы, ворсинки — и со временем забивается. Когда фильтр перекрыт, вода не проходит в насос, и программа отжима не запускается. Обычно фильтр находится внизу на передней панели: откройте лючок, выкрутите пробку и удалите скопившийся мусор. После очистки попробуйте снова включить стирку.</p>
        ...
        <p><strong>4. Иногда проблема связана с загрузкой белья.</strong><br/>Если ��ещи сбились в комок, барабан не может сбалансироваться. Датчик дисбаланса блокирует разгон. Перетряхните бельё, добавьте мелкие вещи или разделите большую партию на две стирки.</p>
    ...
    },
    {
      ...
      content: `<p>Если помпа сгорела или в ней сломана крыльчат��а, машина не сможет слить воду и переход к отжиму невозможен. Признаки: вода остаётся в баке, машинка гудит во время слива или совсем молчит, и часто появляется ошибка слива на дисплее. Самостоятельный ремонт насоса обычно невозможен.</p>`
    },
    {
      id: '3',
