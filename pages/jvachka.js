...
const Page = () => {
  ...
  const structuredData = getArticleStructuredData(
    ...
    'Эффективные методы удаления жвачки с одежды: холод, тепло и мя��кие растворители',
    `${baseUrl}/jvachka`,
  ...
  const sections = [
    ...
    },
    {
      ...
      imageAlt: '',
      imageCaption: '',
      content: `<p>Если ни холод, ни тепло не помогли, пробуйте местные растворители. Жидкость для снятия лака с ацетоном мягко разъедает жевательную резинку – смочите ватный диск, приложите к пятну на 2–3 минуты, затем сотрите резинку щёткой. Ксилол (уайт-спирит) или промышленный очиститель WD-40 тоже эффективно размягчают клейкость жвачки. Эти средства хорошо подходят для плотных тканей, но обязательно протестируйте их на незаметном фрагменте, чтобы избежа��ь обесцвечивания. После применения растворителя одежду нужно обязательно выстирать – это удалит остатки химии и запах.</p>`
    },
    {
      id: '4',
